...
Proof sketch: the total degree $2|E|$ is even. Split the sum into even-degree vertices and odd-degree vertices. The even part is even, so the odd part must also be even. A sum of odd numbers is even only when there's an even count of them. ∎
