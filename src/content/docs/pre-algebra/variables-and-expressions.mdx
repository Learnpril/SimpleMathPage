...
A **variable** is a letter that stands in for a number we don't know yet. We usually use letters like _x_, _y_, or _n_, but any letter works.
...
This means "take some number _x_, multiply it by 3, then add 5." There's no equals sign — it's an expression, not an equation.
...
1. **Identify the variable.** Let _m_ = the number of movies rented.
...
The base fee is the constant (10), and $3m$ represents 3 dollars times however many movies you rent.
...
Algebraic expressions are everywhere once you start looking. Budgeting ("I spend _x_ dollars on coffee each week"), fitness ("I burn $200 + 50t$ calories where _t_ is minutes of running"), and even cooking ("double the recipe" means multiplying every ingredient by 2) all use the same idea — a pattern with a slot you can fill in with different numbers.
...
If the jump from plain numbers to letters feels weird, that's totally normal. Think of a variable as a blank in a sentence — it's just waiting for you to fill it in. Once you see that 3*x* + 5 is really just a recipe ("multiply by 3, add 5"), the rest follows naturally. Practice with small numbers and you'll build confidence fast.
:::

## Quiz

<Quiz
  quizId="algebra-variables-and-expressions"
  questions={[
    {
      id: "q1",
      ...
      correctIndex: 2,
      explanation:
        "The coefficient is the number multiplied by the variable. In 7x, the coefficient is 7.",
    },
    {
      id: "q2",
    ...
    },
    {
      ...
      correctIndex: 1,
      explanation:
        "An expression has no equals sign. '2y - 7' is an expression; the others are equations.",
    },
    {
      id: "q4",
      text: "A gym charges 25 dollars per month plus 5 dollars per class. Which expression represents the monthly cost for c classes?",
      options: ["25c + 5", "5c + 25", "25 + 5 + c", "30c"],
      correctIndex: 1,
      explanation:
        "The fixed cost is 25 dollars (constant) and each class costs 5 dollars, so the expression is 5c + 25.",
    },
    {
      id: "q5",
      text: "Translate into an expression: 'a number decreased by 9'.",
      options: ["$x - 9$", "$9 - x$", "$9x$", "$x + 9$"],
      correctIndex: 0,
      explanation: "'Decreased by' means subtraction: $x - 9$.",
    },
  ]}
/>
