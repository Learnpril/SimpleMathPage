---
title: "Solving Systems with Matrices: The Inverse Method"
description: Write a system of two linear equations as AX = B and solve it with the inverse matrix. Set-up, worked steps, and when the method does not work.
sidebar:
  order: 31
---
...
A system of two linear equations can be written as one matrix equation $AX = B$, then solved with the inverse: $X = A^{-1}B$. This lesson shows how to set it up and work it through.
...
<Quiz
  ...
  questions={[
    ...
    },
    {
      ...
      options: ["$X = A + B$", "$X = AB$", "$X = B - A$", "$X = A^{-1}B$"],
      correctIndex: 3,
