...
Here's what the tree looks like when you have even more variables. This isn't part of the examples above, just a bonus visual to show how the pattern scales. For w = f(x, y, z) where x, y, and z each depend on s and t, the tree gets wider but the rule stays the same: multiply along each path, add all paths to the same bottom variable.

**Bonus: Tree diagram with three intermediate variables**

import ChainRuleTree4Var from "../../../components/ChainRuleTree4Var.astro";
